# Question 1 -- Annapurna Stores billing/revenue

Part of the [Lab 1 submission](../../README.md); see that index for
Question 2 (SetuBid procurement-notice dedup, `../question2/README.md`).
Everything below is Question 1 only.

## Layout

```
data-mining-lab-1/question1/
...
answer. All commands below assume the repo root as the working
directory (matching Question 2's README), not this file's own
directory.
...
```
data-mining-lab-1/question1/masters.sql
data-mining-lab-1/question1/billing_notes.md
data-mining-lab-1/question1/finance_monthly.csv
data-mining-lab-1/question1/sales/*.csv
data-mining-lab-1/question1/_truth/       (optional -- only used for the Task B/F cross-checks)
```

(Question 2's provided-data paths are listed in `../question2/README.md`.)

### Rebuild everything from scratch, in order

```
cd data-mining-lab-1/question1
docker compose up -d
...
```
duckdb question1/duckdb/annapurna.duckdb -init question1/duckdb/setup.sql
```
...
duckdb question1/duckdb/annapurna.duckdb \
    -init question1/duckdb/setup.sql \
    -f    question1/duckdb/load_fact_sales.sql
```
...
## Task C -- tables behind the dashboard (modified star schema)
...
### Why a snowflaked product/category hierarchy?

The dashboard schema is primarily star-oriented, but the product dimension retains a normalized relationship to dim_category. A completely denormalized star schema could copy category_name, gst_rate, and other category-level attributes into dim_product, eliminating the product → category join.

We deliberately retain dim_category because category-level attributes belong to the category entity rather than to individual products. Keeping them in one dimension avoids duplicating category attributes across all product rows and avoids updating those attributes independently across potentially many product/SCD2 records.

This normalization does introduce an additional join, but dim_category is only **14 rows** in this dataset, so its execution cost is negligible compared with processing the **1.12M-row fact table**. Furthermore, the dashboard's main aggregation table, mart_revenue_daily, stores category_id directly, so the common category-revenue path does not need to traverse fact → product → category.

Therefore, the design is intentionally a star schema with a small snowflaked product/category hierarchy, rather than a fully normalized warehouse
...
Each item verified against a live `EXPLAIN`/`EXPLAIN ANALYZE`, not assumed.

1. **Object-level partition pruning** -- Hive layout (`store=/year=/month=`)
   restricts the S3 scan to one prefix. Full scan: 4,457 files (~65.5 MiB);
   store-month: ~31 files (~477 KiB).
2. **Column pruning** -- `READ_CSV` and `POSTGRES_SCAN` both show a
   `Projections:` list limited to the columns the query actually needs.
3. **Early filtering** -- in the federated plan, `FILTER (line_type =
   'SALE')` sits directly on top of `READ_CSV`, before any `HASH_JOIN`.
4. **Hash joins/aggregation** -- DuckDB's plan uses `HASH_JOIN` for the
   sales/products/categories joins and `HASH_GROUP_BY` for the rollup.
5. **Postgres indexes** on `fact_sales_line`: `(store_id, business_date)`,
   `(business_date)`, `(product_sk)`, partial `(category_id,
   business_date) WHERE is_revenue`.
6. **Partial indexing** -- the category index excludes non-revenue rows
   (`TAX`/`TENDER`). Confirmed in use: a category+month query on
   `fact_sales_line` plans a `Bitmap Index Scan on ix_fact_category_date`.
7. **Pre-aggregation** -- `mart_revenue_daily` (65,685 rows) sits in front
   of `fact_sales_line` (1,120,924 rows); dashboard queries hit the mart.
8. **Date dimension** -- `dim_date` precomputes month/quarter/weekday, so
   slicing is a join + `GROUP BY`, never a per-row `EXTRACT()`.
9. **EXPLAIN-based verification** -- every claim above is read off an
   actual plan (see Task C/E and `analytic.sql`), not assumed from the
   schema's existence.
...
```
                RAW DATA
                   |
          +-----------------+
          |  MinIO layout   |  <- #1 partition pruning
          | Hive partition  |
          +--------+--------+
                   |
                   v
          ...
          +-----------------+
          | dialect parsing |
          | deduplication   |  <- #2/#3 column pruning + early filter
          | projection      |
          +--------+--------+
                   |
                   v
          ...
          +-----------------+
          | dimensional     |
          | modelling       |  <- #4 hash joins/aggregation (load-time)
          +--------+--------+
                   |
          +--------v--------+
          |    indexes      |  <- #5/#6 store/date, product, partial category
          +--------+--------+
                   |
          +--------v--------+
          |   daily mart    |  <- #7 pre-aggregation + #8 date dimension
          +--------+--------+
                   |
                   v
               ANALYTICS
...
### Why no curated Parquet layer

Deliberate, not an oversight -- premature at this scale. Parquet's value
(columnar compression, projection/predicate pushdown, fast scans without
a row-store) pays off scanning GBs-TBs; the curated fact table here is
1.12M rows / ~65MB, and Postgres + the indexes above already answer every
target query shape in single-digit-to-tens of milliseconds, verified via
`EXPLAIN ANALYZE`. Adding Parquet on top would be a second derived
artifact to keep idempotent alongside `fact_sales_line`/
`mart_revenue_daily`, for no measurable speedup.

It becomes the right move if either (a) store count or history grows an
order of magnitude or more, so broad scans start to matter more than
indexed point lookups, or (b) DuckDB should be the primary analytical
engine without a live Postgres dependency for the fact table (e.g.
serverless/portable deployments). At that point, curating `sales/` CSVs
into partitioned Parquet in MinIO (same `store=/year=/month=` layout)
would let DuckDB skip both the CSV re-parse and the Postgres round-trip
per query -- the natural next tier if the warehouse outgrows this one,
not needed for the CFO's stated query shapes today.
